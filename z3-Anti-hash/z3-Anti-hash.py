...
[!] Powered By z3morov SEC ( z3eem56 ) 
...
if lk == '1':
    ...
    algorithms = {"102020": "ADLER-32", "102040": "CRC-32", "102060": "CRC-32B", "101020": "CRC-16",
                  ...
                  "107060": "MD5(Unix)", "107020": "MD5(Wordpress)", "108020": "MD5(APR)",
                  "106160": "Haval-128",
                  "106165": "Haval-128(HMAC)", "106060": "MD2", "106120": "MD2(HMAC)", "106040": "MD4",
                  "106100": "MD4(HMAC)", "106020": "MD5", "106080": "MD5(HMAC)",
                  "106140": "MD5(HMAC(Wordpress))",
                  "106029": "NTLM", "106027": "RAdmin v2.x", "106180": "RipeMD-128",
                  "106185": "RipeMD-128(HMAC)",
                  "106200": "SNEFRU-128", "106205": "SNEFRU-128(HMAC)", "106220": "Tiger-128",
                  "106225": "Tiger-128(HMAC)", "106240": "md5($pass.$salt)",
                  ...
                  "109180": "RipeMD-160(HMAC)", "109120": "RipeMD-160", "109020": "SHA-1",
                  "109140": "SHA-1(HMAC)",
                  ...
                  "115120": "SHA-256(HMAC)", "116020": "md5($pass.$salt) - Joomla",
                  ...
                  "122020": "SHA-512", "122060": "SHA-512(HMAC)", "122040": "Whirlpool",
    ...
    def CRC16CCITT(hash):
        hs = '3d08'
        if len(hash) == len(
                hs) and hash.isdigit() == False and hash.isalpha() == False and hash.isalnum() == True:
            jerar.append("101040")
    ...
    def GHash323(hash):
        hs = '80000000'
        if len(hash) == len(
                hs) and hash.isdigit() == True and hash.isalpha() == False and hash.isalnum() == True:
            jerar.append("103040")
    ...
    def MD5APR(hash):
        ...
        if len(hash) == len(hs) and hash.isdigit() == False and hash.isalpha() == False and hash[0:4].find(
                '$apr') == 0:
            jerar.append("108020")
...
if lk == '2':
    ...
    chose = input('''
     ...
     what you would use ?
      ...
      [1] Use Wordlist 
      [2] Generate Words until Crack Hash 

       What Your Chose ? : ''')
    if chose == '2':
        Rand = input("Gusess Word : ")
        v = True
        while v == True:
            types = input("Please Enter the type of hash (md4,md5,sha1,sha512,etc...) : ")
            for i in range(1, 15 ** len(Rand)):
                r = ''.join(random.choice(Rand) for i in range(len(Rand)))
                try:
                    digest = hashlib.new(types)
                    v = False
                except:
                    continue
                digest.update(r.encode())
                f = digest.hexdigest()
                print('[Z] ' + r + ' \t ' + f + ' am keep Working ... ')
                os.system("clear")
                if f == pass_hash:
                    print('\n[3] ' + r + '\t' + f + ' Done !!! ')
                    break
    ...
    if chose == '1':
        ...
        try:
            pass_file = open(wordlist, 'r').readlines()
        except:
            print(" You Type Wrong Path ")
            quit()
        types1 = input("Please Enter the type of hash (md5,sha1,sha512,etc...) : ")
        for i1 in range(len(pass_file)):
            try:
                digest = hashlib.new(types1)

            except:
                continue
            digest.update(pass_file.encode())
            ff = digest.hexdigest()
            print('[Z] ' + pass_file + ' \t ' + ff + ' am keep Working ...')
            os.system("clear")
            if ff == pass_hash:
                print('\n[3] ' + pass_file + '\t' + ff + ' Done !!! ')
                print('''
                
                 [*] thank you so much to use my tool <3
                 [*] my accounts :
                 [+] IG : https://www.instagram.com/z3morovsec
                 [+] TW : https://twitter.com/z3morovsec
                 [+] YT : https://www.youtube.com/channel/UCVhaYMty-wDQsbeJhCf_D5Q''')
